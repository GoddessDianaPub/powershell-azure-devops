#You can print the user's access level by using this PowerShell script.
...
#Prints the user access level
  Foreach($users in $response2.members)
  {
    ...
    { 
       ...
       $response3 = Invoke-RestMethod -Uri $url3 -Headers @{Authorization = "Basic $token"} -Method Get -ContentType application/json-patch+json
       $response3.accessLevel.licenseDisplayName
    }
